...
pub fn bucket_sort_trace(values: &[i32]) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, "bucket-sort")
    ...
    steps.push(
        TraceStep::new(StepType::Select, "create-buckets")
            .with_items(items.clone())
            .with_stats(comparisons, swaps, writes)
            .with_note(format!("创建 {} 个桶", bucket_count)),
    );

    let mut buckets: Vec<Vec<VisualItem>> = vec![Vec::new(); bucket_count];
    for item in items.iter() {
        let idx =
            (((item.value - min_val) as i32 / bucket_size.max(1)) as usize).min(bucket_count - 1);
        buckets[idx].push(item.clone());
        writes += 1;
        steps.push(
            TraceStep::new(StepType::Push, "distribute")
                .with_items(items.clone())
                .with_active(vec![item.id])
                .with_stats(comparisons, swaps, writes)
                .with_note(format!("将 {} 放入桶 {}", item.value, idx)),
        );
    }
    ...
    for (bi, bucket) in buckets.iter_mut().enumerate() {
        ...
        swaps += bucket.len();
        steps.push(
            TraceStep::new(StepType::Select, "sort-bucket")
                .with_items(items.clone())
                .with_active(bucket.iter().map(|it| it.id).collect())
                .with_stats(comparisons, swaps, writes)
                .with_note(format!("对桶 {} 排序", bi)),
        );
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let steps = bucket_sort_trace(&[]);
        assert_eq!(steps.len(), 2);
        assert!(steps.last().unwrap().items.is_empty());
    }
    #[test]
    fn test_single() {
        let steps = bucket_sort_trace(&[42]);
        assert!(is_sorted(&steps.last().unwrap().items));
    }
    #[test]
    fn test_random() {
        let values = vec![78, 17, 39, 26, 72, 94, 21, 12];
        let steps = bucket_sort_trace(&values);
        assert!(is_sorted(&steps.last().unwrap().items));
        assert_eq!(final_values(&steps), vec![12, 17, 21, 26, 39, 72, 78, 94]);
    }
    #[test]
    fn test_duplicate() {
        let values = vec![5, 3, 5, 1, 3];
        let steps = bucket_sort_trace(&values);
        assert!(is_sorted(&steps.last().unwrap().items));
    }
}
