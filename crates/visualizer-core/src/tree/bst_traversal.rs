...
pub fn bst_levelorder_trace(values: &[i32]) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, "levelorder")
    ...
    while let Some(idx) = queue.first().copied() {
        ...
        steps.push(
            TraceStep::new(StepType::Visit, "visit")
                ...
                .with_note(format!("访问节点 {}", items[idx].value)),
        );
        let left = 2 * idx + 1;
        let right = 2 * idx + 2;
        if left < items.len() {
...
fn bst_traversal_trace(values: &[i32], key: &str, f: TraversalFn) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, key)
            .with_items(items.clone())
            .with_note(format!(
                "开始{}遍历",
                match key {
...
fn build_preorder(
    ...
    *seq += 1;
    steps.push(
        TraceStep::new(StepType::Visit, "visit-pre")
            .with_items(items.clone())
            .with_active(vec![items[idx].id])
            .with_note(format!("前序访问: {} (第{}个)", items[idx].value, seq)),
    );
...
fn build_inorder(
    ...
    *seq += 1;
    steps.push(
        TraceStep::new(StepType::Visit, "visit-in")
            .with_items(items.clone())
            .with_active(vec![items[idx].id])
            .with_note(format!("中序访问: {} (第{}个)", items[idx].value, seq)),
    );
...
fn build_postorder(
    ...
    *seq += 1;
    steps.push(
        TraceStep::new(StepType::Visit, "visit-post")
            .with_items(items.clone())
            .with_active(vec![items[idx].id])
            .with_note(format!("后序访问: {} (第{}个)", items[idx].value, seq)),
    );
}
...
mod tests {
    use super::*;
    fn check_done(steps: &[TraceStep]) {
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
    #[test]
    fn test_preorder() {
        check_done(&bst_preorder_trace(&[1, 2, 3, 4, 5]));
    }
    #[test]
    fn test_inorder() {
        check_done(&bst_inorder_trace(&[1, 2, 3, 4, 5]));
    }
    #[test]
    fn test_postorder() {
        check_done(&bst_postorder_trace(&[1, 2, 3, 4, 5]));
    }
    #[test]
    fn test_levelorder() {
        check_done(&bst_levelorder_trace(&[1, 2, 3, 4, 5]));
    }
    #[test]
    fn test_empty_all() {
        for steps in &[
