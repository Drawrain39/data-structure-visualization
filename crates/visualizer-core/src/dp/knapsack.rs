...
pub fn knapsack_trace(values: &[i32]) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, "knapsack")
            .with_items(items.clone())
            .with_extra(serde_json::json!({"capacity":capacity,"items":n}))
            .with_note(format!("0/1 背包: 容量={}, 物品数={}", capacity, n)),
    );
    ...
    if n == 0 {
        steps.push(
            TraceStep::new(StepType::Done, "done")
                .with_items(items)
                .with_extra(serde_json::json!({"result":0}))
                .with_note("无物品，最大价值=0"),
        );
        return steps;
    }
    ...
    for i in 1..=n {
        ...
        for j in 0..=capacity {
            ...
            if j % (capacity.max(1) / 3).max(1) == 0 {
                steps.push(
                    TraceStep::new(StepType::Select, "dp-update")
                        .with_items(items.clone())
                        .with_active(vec![items[i].id])
                        .with_extra(
                            serde_json::json!({"i":i,"j":j,"val":dp[i][j],"capacity":capacity}),
                        )
    ...
    steps.push(
        TraceStep::new(StepType::Done, "done")
            .with_items(items)
            .with_extra(serde_json::json!({"capacity":capacity,"result":result}))
            .with_note(format!("最大价值 = {}", result)),
    );
    steps
}
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let steps = knapsack_trace(&[]);
        assert!(!steps.is_empty());
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
    #[test]
    fn test_basic() {
        let steps = knapsack_trace(&[10, 2, 3, 4, 5]);
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
}
