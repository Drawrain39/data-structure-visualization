...
pub fn topological_sort_trace(values: &[i32]) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, "topo-sort")
            .with_items(items.clone())
            .with_note("拓扑排序 (Kahn 算法)"),
    );
    ...
    steps.push(
        TraceStep::new(StepType::Select, "init-queue")
            .with_items(items.clone())
            .with_active(queue.clone())
            .with_note(format!("入度为 0 的节点入队")),
    );
    ...
    while let Some(u) = queue.first().copied() {
        ...
        steps.push(
            TraceStep::new(StepType::Visit, "process")
                .with_items(items.clone())
                .with_active(vec![items[u].id])
                .with_sorted(order.clone())
                .with_note(format!("处理节点 {}, 输出顺序加入", u)),
        );
        ...
        for &v in &adj[u] {
            ...
            if indegree[v] == 0 {
                queue.push(v);
                steps.push(
                    TraceStep::new(StepType::Push, "enqueue")
                        .with_items(items.clone())
                        .with_active(vec![items[v].id])
                        .with_sorted(order.clone())
                        .with_note(format!("节点 {} 入度为 0，入队", v)),
                );
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let steps = topological_sort_trace(&[]);
        assert!(!steps.is_empty());
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
    #[test]
    fn test_dag() {
        let steps = topological_sort_trace(&[1, 2, 3, 4, 5]);
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
        assert!(!steps.last().unwrap().sorted.is_empty());
    }
}
