...
pub fn kruskal_trace(values: &[i32]) -> Vec<TraceStep> {
    ...
    steps.push(
        TraceStep::new(StepType::Start, "kruskal")
            .with_items(items.clone())
            .with_note("Kruskal 最小生成树"),
    );
        ...
        if find(&mut parent, u) != find(&mut parent, v) {
            ...
            steps.push(
                TraceStep::new(StepType::Select, "select")
                    .with_items(items.clone())
                    .with_swapping(vec![items[u].id, items[v].id])
                    .with_sorted(mst.clone())
                    .with_note(format!("选中边 {}-{}", u, v)),
            );
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let steps = kruskal_trace(&[]);
        assert!(!steps.is_empty());
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
    #[test]
    fn test_mst() {
        let steps = kruskal_trace(&[4, 8, 1, 3, 7]);
        assert_eq!(steps.last().unwrap().step_type, StepType::Done);
    }
}
